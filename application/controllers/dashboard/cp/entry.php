...
class entry extends CP_Controller {
    ...
    function viewport($unit='00000',$type=''){
        ...
        $this->smarty->view( 'dashboard/cp/cdata/viewport', $this->assigns );
    }
    function viewportonly($unit='',$Id){
        ...
        if($Id){
            ...
            if($item) $item->data_data = unserialize($item->data_data);
            $this->assigns->item = $item;
        }
        $layout = $this->assigns->action['layout'];
        switch ($layout){
            default :
                $this->assigns->formhtml = $this->smarty->view( 'dashboard/cp/cdata/editPanel', $this->assigns, true );
        }
        $this->smarty->view( 'dashboard/cp/cdata/viewportonly', $this->assigns );
    }
    function editpanel($type=''){
        ...
        switch ($layout){
            case '1':
                $htmlreponse = $this->smarty->view( 'dashboard/cp/cdata/editPanelGallery', $this->assigns, true );
                break;
            default :
                $htmlreponse = $this->smarty->view( 'dashboard/cp/cdata/editPanel', $this->assigns, true );
        }
    ...
    function loadscript($src='',$unit='00000'){
        ...
        $this->smarty->view( 'dashboard/cp/cdata/'.$src, $this->assigns );
    ...
    function beforecommit(){
        ...
        $item = $this->Core_Model->onGet($Id);
        if($item){
            $data = unserialize($item->data_data);
            $_POST['Params']['data_data'] = array_merge_recursive_extend($data, $_POST['Params']['data_data']);
        }
